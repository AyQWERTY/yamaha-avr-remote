...
import { state, getFeaturesCache, saveFeaturesCache, FEATURES_CACHE_TTL } from './state.js';
...
export function applyStatusData(data) {
  ...
  if (data.tone_control) {
    if (data.tone_control.bass !== undefined) state.bass = data.tone_control.bass;
    if (data.tone_control.treble !== undefined) state.treble = data.tone_control.treble;
  }
...
export async function fetchStatus() {
  ...
  try {
    ...
    if (data.power === undefined && data.volume === undefined && data.input === undefined) {
      return false;
    }
    _lastStatusData = data;
...
export function ensureFeatures() {
  if (!_featuresPromise) {
    ...
    _featuresPromise = (async () => {
      ...
      if (version === _featuresVersion && cached && cached.timestamp
