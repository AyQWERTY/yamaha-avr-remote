...
function getVolumeRange() {
  const min =
    state.volRangeMin !== null && state.volRangeMin !== undefined
      ? parseInt(state.volRangeMin, 10)
      : 0;
  const max =
    state.maxVolume !== null && state.maxVolume !== undefined
...
function rawToDB(raw) {
  if (
    state.volStep === null ||
    state.volStep === undefined ||
    state.volDbMin === null ||
    state.volDbMin === undefined
  ) {
    return null;
  }
...
function setSliderFill(sliderFillEl, pct) {
  ...
  const thumbCenterPx =
    thumbRadiusPx + (pct / 100) * (containerWidth - 2 * thumbRadiusPx);
...
function initVolumeButtons() {
  ...
  function updateVolumeDisplay(raw) {
    ...
    const formatted =
      db === null ? "—" : db >= 0 ? `+${db.toFixed(1)}` : db.toFixed(1);
  ...
  let lastSendRaw = -1;
  let sendTimer = null;
  volumeSliderEl.addEventListener("mousedown", () => {
    dragging = true;
  });
  volumeSliderEl.addEventListener("touchstart", () => {
    dragging = true;
  });
...
function initInputSelect() {
  ...
  inputSelectEl.addEventListener("change", async () => {
    try {
      await sendMessage({
        action: "setInputSource",
        input: inputSelectEl.value,
      });
      await refresh(SETTLE_DELAY);
    } catch (err) {
...
function initExtraBassToggle() {
  ...
  btn.addEventListener("click", () =>
    toggleFeature("extra_bass", "setExtraBass", "extraBassToggle"),
  );
}
...
function initEnhancerToggle() {
  ...
  btn.addEventListener("click", () =>
    toggleFeature("enhancer", "setEnhancer", "enhancerToggle"),
  );
}
...
function syncToggles(statusData) {
  ...
  if (data.extra_bass !== undefined) {
    document
      .getElementById("extraBassToggle")
      .classList.toggle("active", data.extra_bass);
  }
  if (data.enhancer !== undefined) {
    document
      .getElementById("enhancerToggle")
      .classList.toggle("active", data.enhancer);
  }
}
  ...
  const sliders = [
    {
      id: "bassSlider",
      valueId: "bassValue",
      min: state.toneRangeMin,
      max: state.toneRangeMax,
      value: state.bass,
    },
    {
      id: "trebleSlider",
    ...
    },
    {
      id: "subwooferSlider",
      valueId: "subValue",
      min: state.subRangeMin,
      max: state.subRangeMax,
      value: state.subwoofer,
    },
  ];
...
function initToneSlider(action, sliderId, valueId, labelId) {
  ...
  const restoreLabel = () => {
    labelEl.textContent =
      labelEl.dataset.param.charAt(0).toUpperCase() +
      labelEl.dataset.param.slice(1);
  };
  ...
  slider.addEventListener("input", () => {
    ...
    clearTimeout(sendTimer);
    sendTimer = setTimeout(() => {
      sendMessage({ action, value: val }).catch((err) =>
